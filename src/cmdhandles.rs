        ...
        .and_then(|(bot, db, subscriber, feed_link, chat_id)| {
            match db.unsubscribe(subscriber, &feed_link) {
                ...
                Err(Error(ErrorKind::NotSubscribed, _)) => {
                    bot.message(chat_id, "you haven't subscribe this.".to_string())
                        .send()
                }
                Err(e) => {
                    log_error(&e);
...
fn check_channel<'a>(
    ...
    let bot = bot.clone();
    async_block! {
        let msg = await!(bot.message(chat_id, "getting Channel info...".to_string()).send())?.1;
        let msg_id = msg.message_id;
        let channel = match await!(bot.get_chat(channel).send()) {
            Ok((_, channel)) => channel,
            Err(telebot::Error::Telegram(_, err_msg, _)) => {
                let msg = format!("cannot find Channel: {}", err_msg);
        ...
        if channel.kind != "channel" {
            let msg = "target should be a Channel.".to_string();
            await!(bot.edit_message_text(chat_id, msg_id, msg).send())?;
            return Ok(None);
        }
